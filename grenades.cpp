...
void wall(int x, int y) { printarr[x][y] = n * m; }
    ...
    while (!stk_1.empty()) {
      ...
      for (int k = 0; k < 4; ++k) {
        ...
        if (sarr[t.lvl][i][j] != n * m + 1) {
          ...
          } else if (sarr[t.lvl][i][j] == n * m && t.lvl != grens &&
                     sarr[t.lvl + 1][i][j] == n * m) {
...
void show() {
  ...
  RenderWindow window(VideoMode(size * (m + 2), size * (n + 2)),
                      "Maze with grenades.", Style::Titlebar | Style::Close);
